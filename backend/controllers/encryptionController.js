...
class EncryptionController {
  ...
  async decryptOTP(req, res) {
    try {
      ...
      if (!encryptedText || !key) {
        return res
          .status(400)
          .json({ error: "Encrypted text and key are required" });
      }
  ...
  async encrypt3DES(req, res) {
    try {
      ...
      if (!text || !key) {
        return res
          .status(400)
          .json({ error: "Text and key are required", src: "controller" });
      }

      const result = encryptionService.tripleDesEncrypt(text, key);
      res.json(result);
    } catch (error) {
      res.status(400).json({ error: error.message, src: "services" });
    }
  }
  ...
  async decrypt3DES(req, res) {
    try {
      ...
      const decrypted = encryptionService.tripleDesDecrypt(
        encryptedData,
        key,
        iv,
      );
  ...
  async decryptAES(req, res) {
    try {
      ...
      if (!encryptedData || !key || !iv) {
        return res
          .status(400)
          .json({ error: "Encrypted data, key and IV are required" });
      }

      const decrypted = encryptionService.aesDecrypt(
        encryptedData,
        key,
        iv,
        algorithm,
